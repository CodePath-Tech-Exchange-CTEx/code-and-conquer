...
# All logic is mock/placeholder — no real auth provider connected yet.
#
# Flow:
...
def render_sign_in() -> None:
    ...
    with col:
        ...
        username = st.text_input("Username", placeholder="nadha_scholar", key="si_username")

        pw_col, forgot_col = st.columns([3, 2])
        with pw_col:
            st.markdown("**Password**")
        with forgot_col:
            # Render as a styled link — clicking navigates to restore_access screen
            st.markdown(
                """
                <div style="text-align:right; padding-top:6px;">
                  <a href="?auth_screen=restore_access"
                     style="color:#c9a800; font-weight:600; font-size:0.85rem;
        ...
        # Handle restore_access navigation from the Forgot Password link
        if st.query_params.get("auth_screen") == "restore_access":
        ...
        if st.button("Sign In →", use_container_width=True, key="si_submit", type="primary"):
            if not username or not password:
                ...
                # ── Mock auth: store in both session_state AND query params
                # so it survives href-based navigation reruns
                st.session_state["authenticated"] = True
                st.session_state["current_user_id"] = "user-uuid-1"
                st.rerun()
    ...
    with col:
        ...
        if st.button("Send Request Link →", use_container_width=True, key="ra_submit", type="primary"):
            if not email:
                ...
                st.success(f"Reset link sent to {email} (mock — no email actually sent).")
